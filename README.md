...
1. Verify -> the sciprt will be executed once with the action `p4cpm.args.action` as `p4cpm.ACTION_VERIFY`.
2. Change -> the sciprt will be executed twice, once with the action `p4cpm.args.action` as `p4cpm.ACTION_LOGON` and once with the `p4cpm.args.action` as `p4cpm.ACTION_CHANGE`.
3. Reconcile -> the sciprt will be executed twice, once with the `p4cpm.args.action` as `p4cpm.ACTION_PRERECONCILE` and once with the `p4cpm.args.action` as `p4cpm.ACTION_RECONCILE`.
